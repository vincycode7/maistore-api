...
class ProductSizeModel(db.Model, ModelsHelper):
    ...
    def check_unique_inputs(cls, size_data=None):
        ...
        productcat = cls.find_productcat_by_id(productcatid=size_data.get("productcatid", None))
        productsubcat = cls.find_productsubcat_by_id(productsubcatid=size_data.get("productsubcatid", None))
    ...
    def post_unique_already_exist(cls, claim, size_data):
        ...
        productsize, productcat, productsubcat = cls.check_unique_inputs(size_data=size_data)
        ...
        if not productcat:
            return {
                "message": DOES_NOT_EXIST.format("product category" + " " + str(size_data["productcatid"]))
            }, 400  # 400 is for bad request

        #check if productid exist
        if not productsubcat:
            return {
                "message": DOES_NOT_EXIST.format("product sub category" + " " + str(size_data["productsubcatid"]))
            }, 400  # 400 is for bad request
        ...
        if productsubcat.productcat.id != productcat.id:
            return {
                "message": NOT_FOUND_IN.format("product sub category", "category id " + str(size_data["productsubcatid"]))
            }, 400  # 400 is for bad request
