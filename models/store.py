...
class StoreModel(db.Model,ModelsHelper):
    ...
    products = db.relationship("ProductModel", lazy="dynamic", backref="store", cascade="all, delete-orphan")
    customers = db.relationship("FavStoreModel", lazy="dynamic", backref="store", cascade="all, delete-orphan")
    orders = db.relationship("CartSystemModel", lazy="dynamic", backref="store", cascade="all, delete-orphan")
    locations = db.relationship("StorelocModel", lazy="dynamic", backref="store", cascade="all, delete-orphan")
    phonenos = db.relationship("StorephoneModel", lazy="dynamic", backref="store", cascade="all, delete-orphan")
    emails = db.relationship("StoreemailModel", lazy="dynamic", backref="store", cascade="all, delete-orphan")
    ...
    def post_unique_already_exist(cls, claim, store_data):
        ...
        if not user: 
            ...
            return { "message": ALREADY_EXISTS.format("store", store_data["storename"])}, 401  # 400 is for bad request
        return False, 200
    ...
    def put_unique_already_exist(cls, claim, storeid, store_data):
        ...
        if not claim["is_admin"] and claim["userid"] != store_data["user_id"]: 
            return store, {"message": ADMIN_PRIVILEDGE_REQUIRED.format("edit user data")}, 401

        storename, user = cls.check_unique_inputs(store_data=store_data)
        if not user: 
            return store, { "message": NOT_FOUND.format("user")}, 400
        elif store and storename and store.storename != storename.storename:
            return store, {"message": ALREADY_EXISTS.format("storename", store_data["storename"])}, 400  # 400 is for bad request
